...
class TrezorEvmSigner(
    ...
    /**
     * Signs an EIP-191 `personal_sign` message via the Trezor Suite deep-link
     * (`ethereumSignMessage`). Returns `r‖s‖recId` (recId ∈ {0,1}), byte-identical to mnemonic
     * signing, so callers can treat hardware and software signatures uniformly.
     */
    suspend fun signPersonalMessage(message: ByteArray): ByteArray {
        val params = buildJsonObject {
            put("path", derivationPath)
            put("message", message.toHexString())
            put("hex", true)
        }
        val payload = deepLinkManager.call(TrezorMethod.EthSignMessage, params).requirePayload()
        // ethereumSignMessage -> { address, signature (hex, 65 bytes: r || s || v) }
        val sig = payload.hexBytes("signature")
    ...
    /** Not supported: Trezor Connect has no raw-hash (`eth_sign`) operation for Ethereum. */
    ...
    /** Not supported: Trezor Connect's typed-data signing is not wired into this deep-link flow. */
