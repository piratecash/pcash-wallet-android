    ...
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        db.execSQL(
            "UPDATE EnabledWallet " +
                    "SET coinName = (SELECT coinName FROM CustomToken WHERE CustomToken.coinType = EnabledWallet" +
                    ".coinId), " +
                    "coinCode = (SELECT coinCode FROM CustomToken WHERE CustomToken.coinType = EnabledWallet.coinId)," +
                    " " +
                    "coinDecimals = (SELECT decimal FROM CustomToken WHERE CustomToken.coinType = EnabledWallet" +
                    ".coinId) " +
