...
- **Убраны избыточные соединения для Bitcoin / Litecoin при использовании разных типов адресов**
  Оптимизировано количество сетевых подключений при работе с разными типами BTC и LTC адресов, что позволило снизить потребление ресурсов и сетевого трафика при наличии нескольких типов адресов.
...
- **Исправлен краш при сохранении аккаунта после истечения аутентификации Keystore**
  Приложение могло падать с UserNotAuthenticatedException при попытке сохранить или обновить аккаунт в базе данных после истечения аутентификации Keystore. Проблема возникала редко и только на некоторых устройствах.

- **Исправлен краш SQLiteDatabaseCorruptException в Zcash SDK**
  Приложение могло падать из-за SQLiteDatabaseCorruptException при сборе данных из внутренней базы данных Zcash SDK. Краш происходил в методе queryAndMap во время работы синхронизатора.

- **Исправлен краш при переходе на экран Hidden Wallet Terms после покупки Premium**
  Приложение могло падать с IllegalStateException при навигации на экран hidden_wallet_terms_page после возврата с экрана покупки Premium.

- **Исправлен краш приложения при использовании виджетов рынка**
  Приложение могло падать с IllegalStateException при использовании Market widgets на главном экране.

- **Исправлен краш при сохранении балансов кошельков: FOREIGN KEY constraint failed**
  Приложение могло падать в фоновом потоке при сохранении данных о балансах кошельков в кэш из-за ошибки ограничения внешнего ключа в базе данных.

- **Исправлены проблемы с синхронизацией SOL**
  Устранена проблема, при которой Solana корректно отображалась на главной странице, но при открытии становилась недоступной (серый экран) и транзакции не могли быть обработаны.
