...
class DumpManagerTest {
    ...
    private val tablesCreation =
        "CREATE TABLE IF NOT EXISTS `BlockchainEntity` (`uid` TEXT NOT NULL, `name` TEXT NOT NULL, `eip3091url` TEXT, PRIMARY KEY(`uid`));\n" +
            ...
            "`marketCapRank` INTEGER, `coinGeckoId` TEXT, `image` TEXT, `priority` INTEGER NULL, PRIMARY KEY(`uid`));\n" +
            "CREATE TABLE IF NOT EXISTS `TokenEntity` (`coinUid` TEXT NOT NULL, `blockchainUid` TEXT NOT NULL, `type` TEXT NOT NULL, " +
            "`decimals` INTEGER, `reference` TEXT NOT NULL, PRIMARY KEY(`coinUid`, `blockchainUid`, `type`, `reference`), " +
            "FOREIGN KEY(`coinUid`) REFERENCES `Coin`(`uid`) ON UPDATE NO ACTION ON DELETE CASCADE , " +
            "FOREIGN KEY(`blockchainUid`) REFERENCES `BlockchainEntity`(`uid`) ON UPDATE NO ACTION ON DELETE CASCADE )\n"
    ...
    fun getInitialDump_quotedAndNullFields_escapesQuotesAndFormatsNulls() {
        ...
        val expected = tablesCreation +
            "INSERT OR REPLACE INTO BlockchainEntity VALUES('chain''s-uid','Chain''s Name','https://etherscan.io/tx''s');\n" +
            "INSERT OR REPLACE INTO BlockchainEntity VALUES('no-url-chain','No Url Chain',null);\n" +
            "INSERT OR REPLACE INTO Coin VALUES('coin''s-uid', 'Coin''s Name', 'cus', 5, 'gecko''s-id', 'https://img''s.png', 3);\n" +
            "INSERT OR REPLACE INTO Coin VALUES('null-coin', 'Null Coin', 'nul', null, null, null, null);\n" +
            "INSERT OR REPLACE INTO TokenEntity VALUES('token''s-coin','token''s-chain','eip20''s-type',18,'0xAbc''s123');\n" +
